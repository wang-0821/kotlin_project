...
abstract class BaseDatabase(
    ...
    fun name(): String {
        return config.name + xiao.databse.BaseDatabase.Companion.DATABASE_SUFFIX
    }
    ...
    private fun createSqlSessionFactory(): SqlSessionFactory {
        ...
        setEnvironment(config.name + xiao.databse.BaseDatabase.Companion.ENVIRONMENT_NAME_SUFFIX, configuration, url, username, password)
        return xiao.databse.KtManagedSqlSessionFactory(configuration)
    }
    ...
    private fun scanXmlMappers(configuration: Configuration, mapperXmlPath: String) {
        ...
        val path = if (mapperXmlPath.startsWith(xiao.databse.BaseDatabase.Companion.CLASSPATH_INCLUDE_JAR_PREFIX)) {
            mapperXmlPath.substring(xiao.databse.BaseDatabase.Companion.CLASSPATH_INCLUDE_JAR_PREFIX.length)
        } else {
            mapperXmlPath
        }
    ...
    private fun setEnvironment(
        ...
        val config = HikariConfig().apply {
            ...
            jdbcUrl = databaseUrl
            maximumPoolSize = xiao.databse.BaseDatabase.Companion.MAXIMUM_POOL_SIZE
            minimumIdle = xiao.databse.BaseDatabase.Companion.MINIMUM_IDLE
            connectionTimeout = xiao.databse.BaseDatabase.Companion.CONNECTION_TIMEOUT
            idleTimeout = xiao.databse.BaseDatabase.Companion.IDLE_TIMEOUT
            initializationFailTimeout = 0
        }

        configuration.environment = Environment(
            name,
            xiao.databse.KtManagedTransactionFactory(),
