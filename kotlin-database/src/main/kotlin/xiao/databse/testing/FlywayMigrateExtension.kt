...
class FlywayMigrateExtension : BeforeAllCallback {
    ...
    private fun migrateDatabase(context: ExtensionContext) {
        ...
        TestDataSourceContainer.addDatabaseAnnotations(annotations)
        migrateDatabase(annotations.map { resolveDatabase(it.database) })
    }

    private fun migrateDatabase(databases: List<xiao.databse.BaseDatabase>) {
        ...
        databases.forEach { database ->
            ...
            if (matcher.find() && matcher.groupCount() > 1) {
                Flyway
                    ...
                    .load()
                    .migrate()
                TestDataSourceContainer.addMigratedDatabase(database.name())
    ...
    private fun resolveDatabase(databaseClass: KClass<out xiao.databse.BaseDatabase>): xiao.databse.BaseDatabase {
        return TestDataSourceContainer.databaseInstances[databaseClass] ?: kotlin.run {
            databaseClass.java.newInstance()
                .also {
                    TestDataSourceContainer.addDatabaseInstance(databaseClass, it)
                }
        }
    }
